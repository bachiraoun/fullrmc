...
class weightedRandomSelector(RandomSelector):
    ...
    def set_weights(self, weights): 
        ...
        if weights is not None:
            for w in weights:
                assert isinstance(w, (list,set,tuple)),log.LocalLogger("fullrmc").logger.error("weights list items must be tuples")
                assert len(w)==2, log.LocalLogger("fullrmc").logger.error("weights list tuples must have exactly 2 items")
                idx  = w[0]
                wgt = w[1]
                assert is_integer(idx), log.LocalLogger("fullrmc").logger.error("weights list tuples first item must be an integer")
                idx = INT_TYPE(idx)
                assert idx>=0, log.LocalLogger("fullrmc").logger.error("weights list tuples first item must be positive")
                assert idx<len(self.engine.groups), log.LocalLogger("fullrmc").logger.error("weights list tuples first item must be smaller than engine's number of groups")
                assert is_number(wgt), log.LocalLogger("fullrmc").logger.error("weights list tuples second item must be an integer")
                wgt = FLOAT_TYPE(wgt)
                assert wgt>=0, log.LocalLogger("fullrmc").logger.error("weights list tuples first item must be positive")
    ...
    def set_group_weight(self, groupWeight):
        ...
        """
        assert isinstance(groupWeight, (list,set,tuple)), log.LocalLogger("fullrmc").logger.error("groupWeight must be a tuple")
        assert len(groupWeight)==2, log.LocalLogger("fullrmc").logger.error("groupWeight must have exactly 2 items")
        idx  = groupWeight[0]
        wgt = groupWeight[1]
        assert is_integer(idx), log.LocalLogger("fullrmc").logger.error("groupWeight first item must be an integer")
        idx = INT_TYPE(idx)
        assert idx>=0, log.LocalLogger("fullrmc").logger.error("groupWeight first item must be positive")
        assert idx<len(self.engine.groups), log.LocalLogger("fullrmc").logger.error("groupWeight first item must be smaller than engine's number of groups")
        assert is_number(wgt), log.LocalLogger("fullrmc").logger.error("groupWeight second item must be an integer")
        wgt = FLOAT_TYPE(wgt)
        assert wgt>=0, log.LocalLogger("fullrmc").logger.error("groupWeight first item must be positive")
        # update groups weight
